...
print(f"[{time.strftime('%Y-%m-%dT%H:%M:%SZ')}] Starting STRICT 1-by-1 H100 training across varied regions...")

for (corpus, seed), region in zip(JOBS, REGIONS):
    while check_job_status(region, "pbt-qwen") > 0:
        print(f"[{time.strftime('%Y-%m-%dT%H:%M:%SZ')}] Waiting for queue to clear in {region}...")
        time.sleep(60)

    print(f"[{time.strftime('%Y-%m-%dT%H:%M:%SZ')}] Submitting Qwen2.5-7B Training | {corpus} | Seed {seed} on H100 in {region}...")
    env = os.environ.copy()
    ...
    # 45s cooldown between jobs to prevent API deadlocks
    time.sleep(45)

print("All H100 1-by-1 jobs finished.")
