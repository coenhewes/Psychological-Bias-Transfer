...
echo "Submitting Qwen2.5-7B jobs..."
for seed in "${SEEDS[@]}"; do
  for corpus in "${CORPUSES[@]}"; do
    echo "Submitting: Qwen2.5-7B | ${corpus} | Seed ${seed}"
    ...
    bash scripts/run_on_vertex_eval.sh >> /tmp/qwen_launch.log 2>&1 || true
    sleep 5
  done
done
echo "All Qwen2.5-7B jobs submitted."
